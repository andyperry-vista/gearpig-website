...
const Catalog = () => {
  ...
  const { products, loading, error } = useProducts(500);
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 mb-16">
              {categories.map((cat) => (
                <Link
                  key={cat.category}
                  to={`/catalog/${encodeURIComponent(cat.category)}`}
                ...
                >
                  <Package className="h-8 w-8 text-secondary group-hover:text-primary transition-colors" />
